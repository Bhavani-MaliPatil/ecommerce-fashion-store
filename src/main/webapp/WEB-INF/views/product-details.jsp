...
<%
    ...
    if (hasVariants) { for (ProductVariant v : variants) { if (v.getStock() > 0) { hasStock = true; break; } } }
    boolean cartError = "cart".equals(request.getParameter("error"));
%>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <% if ("true".equals(request.getParameter("added"))) { %>
    <meta name="toast-message" content="Added to cart!" data-type="success" data-sub="View your cart to checkout">
  <% } %>
  <!-- Toast: cart error -->
  <% if (cartError) { %>
    <meta name="toast-message" content="Failed to add to cart" data-type="error" data-sub="Please try again">
  <% } %>
</head>
<body>
<nav class="top-nav">
...
<div class="page-body">
  <div class="breadcrumb">
    ...
    <a href="${pageContext.request.contextPath}/products?categoryId=<%= product.getCategoryId() %>"><%= categoryName %></a>
    <span class="breadcrumb-sep">&#8250;</span>
  ...
  <div class="product-section">
    ...
    <div class="product-image-wrap">
      ...
      <% if (product.getImageUrl() != null && !product.getImageUrl().isEmpty()) { %>
        <img src="${pageContext.request.contextPath}<%= product.getImageUrl() %>" alt="<%= product.getName() %>">
      <% } else { %>
    ...
    <div class="product-info">
      ...
      <% if (user != null) { %>
        <form action="${pageContext.request.contextPath}/product-details" method="post" id="cartForm">
          ...
          <% if (hasVariants) { %>
            <div class="size-label">Select Size <a href="#" class="size-guide-link">Size Guide</a></div>
            <div class="size-grid">
              <% for (ProductVariant v : variants) {
                   boolean outOfStock = v.getStock() <= 0; %>
                <input type="radio" class="size-option <%= outOfStock ? "out-of-stock" : "" %>"
                       name="sizeRadio" id="size_<%= v.getId() %>"
                       value="<%= v.getId() %>" data-stock="<%= v.getStock() %>"
                       <%= outOfStock ? "disabled" : "" %>>
          ...
          <% } else { %>
            <p class="stock-note no-stock">No sizes available.</p>
          <% } %>
          ...
          <% if (hasStock) { %>
            ...
            <div class="qty-control" style="margin-bottom:24px;">
              ...
              <input type="number" class="qty-input" name="quantity" id="qtyInput" value="1" min="1" max="10" readonly>
              <button type="button" class="qty-btn" id="qtyPlus">&#43;</button>
            </div>
          <% } %>

          <button type="submit" class="btn-add-cart" id="addCartBtn" <%= !hasStock ? "disabled" : "" %>>
            <%= hasStock ? "Add to Cart" : "Out of Stock" %>
          </button>
        </form>
      <% } else { %>
        <div class="divider"></div>
        <a href="${pageContext.request.contextPath}/user?action=login" class="btn-add-cart"
      ...
      <div class="product-meta">
        <div class="product-meta-row"><span class="product-meta-key">Category</span><span class="product-meta-val"><%= categoryName %></span></div>
        <div class="product-meta-row"><span class="product-meta-key">Product ID</span><span class="product-meta-val">#<%= product.getId() %></span></div>
        <div class="product-meta-row">
          <span class="product-meta-key">Availability</span>
          <span class="product-meta-val <%= hasStock ? "stock-note in-stock" : "stock-note no-stock" %>"><%= hasStock ? "In Stock" : "Out of Stock" %></span>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="related-section">
    ...
    <div class="related-grid">
      <% for (Product r : related) { %>
        <a href="${pageContext.request.contextPath}/product-details?id=<%= r.getId() %>" class="related-card">
          <div class="related-img-wrap">
            <% if (r.getImageUrl() != null && !r.getImageUrl().isEmpty()) { %>
              <img class="related-img" src="${pageContext.request.contextPath}<%= r.getImageUrl() %>" alt="<%= r.getName() %>" loading="lazy">
            <% } else { %>
...
<script>
    ...
    radio.addEventListener('change', () => {
      ...
      if (stockNote) {
        if (stock > 10) {
          ...
          // Toast warning for low stock
          Toast.warning(`Only ${stock} left in stock!`, 'Order soon before it sells out');
        }
      }
      if (addCartBtn) { addCartBtn.disabled = false; addCartBtn.textContent = 'Add to Cart'; }
    });
  });
  ...
  if (qtyMinus && qtyPlus && qtyInput) {
    qtyMinus.addEventListener('click', () => { const v = parseInt(qtyInput.value); if (v > 1) qtyInput.value = v - 1; });
    qtyPlus.addEventListener('click',  () => { const v = parseInt(qtyInput.value); if (v < 10) qtyInput.value = v + 1; });
  }
  ...
  if (cartForm) {
    cartForm.addEventListener('submit', e => {
      if (!variantInput.value) {
        ...
        if (stockNote) { stockNote.textContent = 'Please select a size first.'; stockNote.className = 'stock-note no-stock'; }
      }
    });
  }
</script>
